...
| `report.unit_metrics` | If it runs per unit: at least one comparable scalar. Without it a per-unit plugin delivers N single-sample reports stapled together, and whether the units agree is answered nowhere. One plugin's count ran 8,194 to 38,895 across ten animals of one tissue. |
